...
import { t as n } from "./band-p1VbTxZM.js";
import { t as r } from "./select-BIuoFcZG.js";
import { n as i, t as a } from "./axis-EpX3kBPs.js";
...
function u(u, f) {
	function p() {
		r(f).selectAll("svg").remove();
		let p = f.clientWidth || o, m = f.clientHeight || s, h = p - c.left - c.right, g = m - c.top - c.bottom, _ = n().domain(u.categories).range([0, h]).padding(.2), v = n().domain(u.series.map((e) => e.name)).range([0, _.bandwidth()]).padding(.1), y = Math.max(0, ...u.series.flatMap((e) => e.values)), b = e().domain([0, y]).nice().range([g, 0]), x = t().domain(u.series.map((e) => e.name)).range(l), S = r(f).append("svg").attr("viewBox", `0 0 ${String(p)} ${String(m)}`).attr("width", p).attr("height", m), C = S.append("g").attr("transform", `translate(${String(c.left)}, ${String(c.top)})`);
